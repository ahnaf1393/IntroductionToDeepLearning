...
class Classifier(Network):
    ...
    def backward(self, y):
        ...
        # (-->use the function self.sigmoid() here)                            #
        ########################################################################

        sub = 1 - self.sigmoid(np.dot(self.cache, self.W))
        dW = np.multiply(np.multiply(self.sigmoid(np.dot(self.cache, self.W)), sub), self.cache)
