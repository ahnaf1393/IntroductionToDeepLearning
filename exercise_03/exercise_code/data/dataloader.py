...
class DataLoader:
    ...
    def __iter__(self):
        ...
        reminder = len(self.dataset) % self.batch_size
        ...
        for index in index_iterator:  # iterate over indices using the iterator
            ...
            if len(batch) == self.batch_size:
                ...
                batch = []
                count = count + 1
            if index == ds_len - 1 and not self.drop_last and reminder != 0:
                batch_dict = {}
                for data_dict in batch:
                    for key, value in data_dict.items():
                        if key not in batch_dict:
    ...
    def __len__(self):
        ...
        if self.drop_last:
            length = int((len(self.dataset) - (len(self.dataset) % self.batch_size)) / self.batch_size)

        else:
            length = int(((len(self.dataset) - (len(self.dataset) % self.batch_size)) / self.batch_size) + 1)
